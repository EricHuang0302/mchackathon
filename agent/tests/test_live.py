...
def test_model_output_only_proposes_allowlisted_observations():
    ...
    provider._accept_text('{"observations":[{"key":"treatment","value":"start CPR"},{"key":"breathing_reported","value":"start CPR"},{"key":"breathing_reported","value":"unknown"}]}')
    proposals = provider.poll()
    assert len(proposals) == 1
    assert proposals[0]["key"] == "breathing_reported"
