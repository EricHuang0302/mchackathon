...
A Firestore-backed implementation would write
``incidents/{incidentId}/events/{eventId}`` through the Admin SDK with the
event ID as the document ID, which gives the same create-if-absent idempotency
the in-memory store provides. That implementation is intentionally absent on
this branch: nothing here needs a live database, and an untestable adapter
would be worse than a documented seam.
"""
...
class EventStore(ABC):
    ...
    def purge_expired(self, now: datetime) -> int:
        ...
        Retention deletion is the only removal path. It is asynchronous in
        Firestore, so authorization must already deny access at ``expiresAt``
