...
The matrix below is the service-side twin of ``firebase/firestore.rules``.
Admin SDK writes bypass Security Rules entirely, so the backend has to apply an
equivalent check itself rather than relying on the client rules as the only
boundary (``docs/sdd.md`` section 10.2).
"""
...
# Firestore rules tests and in the documentation table.
...
class GrantStore(ABC):
    ...
    A Firestore-backed implementation would read
    ``incidents/{incidentId}/grants/{uid}``; see ``firebase/README.md`` for why
    grants are keyed by uid rather than by a random grant ID.
    """
