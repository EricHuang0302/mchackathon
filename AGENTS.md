...
- The backend uses **Python 3.12, Flask, Google ADK, and Google Gemini** in `agent/`. Structured application operations use RESTful JSON through Nginx; a dedicated WebSocket carries Live media and control messages. Trusted HTTPS is required for phone access over a LAN. PostgreSQL provides local persistence and scoped reads; Docker Compose runs Nginx, the API, and PostgreSQL locally. Gemini Live and Google Maps are optional external integrations.
...
5. Shared API / WebSocket types belong to workstream 1 with frontend consumers involved; rule schemas and fixtures belong to workstream 5 with workstream 3 involved. The frontend shell and common UI belong to workstream 2; local deployment configuration belongs to workstream 5.
