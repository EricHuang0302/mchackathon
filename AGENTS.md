...
- Call mode is controlled through explicit UI actions. The hackathon prototype opens the configured prototype link `tel:035216121` (Hsinchu City Government switchboard), never `tel:119`, and first mutes the Agent; launching the link records only an attempted call. A user action confirms connection, reports that dispatcher guidance ended, or reports that the call was unavailable. Do not promise automatic telephone-state detection or automatic speakerphone control from the browser.
