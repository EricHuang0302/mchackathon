...
export function DemoHandoffPage({ incidentId }: { incidentId: string }) {
  ...
  return <Stack spacing={3} className="helper-page-enter">
    ...
    <Card><CardContent><Typography variant="h5">完整時間軸</Typography><ol className="handoff-timeline">{demoTimeline.map((event) => <li key={event.id}><time>{formatTime(event.occurredAt)}</time><span>{event.label}<small>{event.confirmation === "confirmed" ? "已確認" : "現場回報"}</small></span></li>)}</ol></CardContent></Card>
